...
public class Ant extends Agent
{
	...
	{
		// get reference to the map agent
		...
		// add behaviour
		addBehaviour(new AntBehaviour());
	}

	private int initMapSize()
	{
		ACLMessage request = new ACLMessage(ACLMessage.REQUEST);
		request.setContent("MapSize?");
	...
	class AntBehaviour extends Behaviour
	{
		...
		public void action()
		{
			...
			case 1:
			{
				...
				ACLMessage request = new ACLMessage(ACLMessage.REQUEST);
				potentialNodeIndices = ant.getPotentialNodeIndices().trim();
				currentMapPosIndex = ant.getCurrentMapPosIndex();
			...
			case 2:
			{
				...
				if (msg != null)
				{
					// response contains a header and alternating numbers designating pheromone
					// level and edge weight for every applicable edge
					...
					for (int i = 1; i < parts.length; i += 2)
					{
						...
						float val = (float) (Math.pow(pheromoneLevel, alpha) * Math.pow(1 / weight,
								beta));
						total += val;
					...
					// update next node index (using chosen pheromone index i, and
					// potentialNodeIndices string)
					String[] segs = potentialNodeIndices.split(" ");
					newNodeIndex = Integer.parseInt(segs[i - 1]);
					ant.setCurrentMapPosIndex(newNodeIndex);

					// add new node to tourSoFar
					ant.addNodeToTour(newNodeIndex);

					// add new weight to tourSoFarWeights
					ant.addWeightToTour(weights.get(i));

					// initiate local pheromone update
					ACLMessage localUpdate = new ACLMessage(ACLMessage.INFORM);
					localUpdate.addReceiver(mapAID);
					localUpdate.setContent("UpdateLocalPheromone " + currentMapPosIndex + " "
							+ newNodeIndex + " " + ksi);

					// advance the phase as required (if tour complete, continue with phase 3,
					// otherwise, repeat phase 1)
					if (ant.getTourSoFarSize() == ant.getMapSize())
					{
						int firstMapPosIndex = ant.getFirstMapPosIndex();
						ant.addNodeToTour(firstMapPosIndex);

						ACLMessage edgeWeightReq = new ACLMessage(ACLMessage.REQUEST);
						...
						ant.setCurrentMapPosIndex(firstMapPosIndex);
			...
			case 3:
			{
				...
				if (msg != null)
				{
					ant.addWeightToTour(Float.parseFloat(msg.getContent()));
			...
			case 4:
			{
				ACLMessage updateBest = new ACLMessage(ACLMessage.INFORM);
				updateBest.addReceiver(mapAID);
				StringBuilder tourSoFar = new StringBuilder();
				for (int i = 0; i < ant.getTourSoFarSize(); ++i)
					tourSoFar.append(" ").append(ant.getTourNode(i));
				float tourWeight = ant.getTotalWeightSoFar();
				updateBest.setContent("UpdateBestTour " + tourWeight + tourSoFar.toString());
				send(updateBest);

				delta = 1 / tourWeight;

				ant.removeLastNode(); // which is the same as the first
			...
			default: // phase == 5
			{
				...
				int nextNodeIndex = ant.removeLastNode();
				...
				if (nextNodeIndex == -1)
				{
					...
					try
					{
						...
						ac.createNewAgent("Ant" + getAID().hashCode() + System.currentTimeMillis(),
								"Ant", null).start();
				...
				ACLMessage updatePheromone = new ACLMessage(ACLMessage.INFORM);
				updatePheromone.addReceiver(mapAID);
				// float val = (1 - ro) * oldValue + ro * delta; // final formula is constructed in
				// Map agent (for simplicity of oldValue retrieval)
				updatePheromone.setContent("UpdatePheromone " + currentMapPosIndex + " "
						+ nextNodeIndex + " " + (1 - ro) + " " + ro * delta);
				send(updatePheromone);

				ant.setCurrentMapPosIndex(nextNodeIndex);
