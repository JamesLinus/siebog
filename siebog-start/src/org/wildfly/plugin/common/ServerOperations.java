...
public class ServerOperations extends Operations
{
	...
	/**
	 * Parses the result and returns the failure description. If the result was successful, an empty
	 * string is returned.
	 *
		...
		if (result.hasDefined(ClientConstants.FAILURE_DESCRIPTION))
		{
			if (result.hasDefined(ClientConstants.OP))
			{
				msg = String.format("Operation '%s' at address '%s' failed: %s",
						result.get(ClientConstants.OP), result.get(ClientConstants.OP_ADDR),
						result.get(ClientConstants.FAILURE_DESCRIPTION));
			} else
			{
	...
	/**
	 * Creates an operation to read the attribute represented by the {@code attributeName}
	 * parameter.
	 *
	 ...
	 *
	 * @throws IllegalArgumentException if the address is not of type {@link ModelType#LIST} or is
	 *             empty
	 */
	...
	/**
	 * Reads the result of an operation and returns the result as a string. If the operation does
	 * not have a {@link ClientConstants#RESULT} attribute and empty string is returned.
	 *
	 * @param result the result of executing an operation
	 *
