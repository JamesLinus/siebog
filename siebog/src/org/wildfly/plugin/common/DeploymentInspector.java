...
public class DeploymentInspector
{
	...
	/**
	 * Lookup an existing Deployment using a static name or a pattern. At least exactComparisonName
	 * or deploymentNamePattern must be set.
	 *
	 * @param client
	...
	public static List<String> getDeployments(ModelControllerClient client,
			String exactComparisonName, String matchPattern)
	{

		if (exactComparisonName == null && matchPattern == null)
		{
			throw new IllegalArgumentException(
					"exactComparisonName and matchPattern are null. One of them must "
		...
		try
		{
			...
			if (Operations.isSuccessfulOutcome(listDeploymentsResult))
			{
				...
				{
					final List<ModelNode> deployments = ServerOperations.readResult(
							listDeploymentsResult).asList();
			...
			} else
			{
				throw new IllegalStateException(
						ServerOperations.getFailureDescriptionAsString(listDeploymentsResult));
			}
		} catch (IOException e)
		{
			throw new IllegalStateException(String.format("Could not execute operation '%s'", op),
					e);
		}
	...
	private static boolean matches(String deploymentName, String exactComparisonName,
			String matchPattern)
	{
