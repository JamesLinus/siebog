class XJAF
	...
	this.post = (msg, onSuccess, onError) ->
		$.ajax "rest/messages"
			...
			success: onSuccess
			error: onError
	this.accept = (url, aid, state, onSuccess, onError) ->
		$.ajax 
		...
		if not running.hasOwnProperty(newAid.str)
			...
			agent.worker.onmessage = (ev) ->
				...
				switch msg.opcode
					...
					when OpCode.MOVE_TO_SERVER
						url = running[msg.aid.str]?.url
						if url?
							XJAF.accept(url, msg.aid.str, msg.state)
					else
						alert("Unrecognized message: " + JSON.stringify(msg))
